...
void WINAPI DllInit()
{
	if (GetModuleHandleA("dxgi.dll"))
	{
		...
		uintptr_t Flag = (uintptr_t)sigscan::scan("LimboPlayer.exe", "\x74\x10\x80\x3D\x00\x00\x00\x00\x00\xC7", "xxxx????xx"); // 74 10 80 3D ?? ?? ?? ?? 00 C7
		if (!Flag)
		{
		...
		SwapChainDesc.OutputWindow = FindWindowW(NULL, L"ROBLOX"); // limbo hooks FindWindowA lol
